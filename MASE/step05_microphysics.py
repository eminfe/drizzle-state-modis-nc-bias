# =============================================================================
# step05_microphysics.py  -  MASE 2005
# =============================================================================
...
def compute_re_cas(gm, var_map=config.VAR_MAP):
    ...
    # Use the campaign-aware column finder (handles spaced 1-indexed names
    # like MASE 'CAS Bin 1' as well as VOCALS 'CAS_bin_00') and align the
    # diameter array with the actual columns by their numeric bin index.
    cas_cols_all = get_cas_columns(gm, cas_prefix)
    if not cas_cols_all:
        gm["re_cas"] = np.nan
        return gm, [], np.array([])
    # Diameter array is laid out in the same physical order as the bins;
    # select_active_bins drops NaN midpoints (inactive instrument bins).
    D_cas_all = np.array(config.CAS_D_MID_ALL[:len(cas_cols_all)], dtype=float)
    D_cas, cas_cols = select_active_bins(D_cas_all, cas_cols_all, cutoff_um=None)
...
def compute_re_full(gm, cas_cols, r_cas, var_map=config.VAR_MAP,
    ...
    cip_cols_all = get_cip_columns(gm, cip_prefix)
    if not cip_cols_all:
        gm["re_full"] = gm["re_cas"]
        return gm

    D_cip_all = np.array(cip_d_mid_um[:len(cip_cols_all)], dtype=float)
    D_cip, cip_cols = select_active_bins(D_cip_all, cip_cols_all, cutoff_um=None)
    if len(cip_cols) == 0:
        gm["re_full"] = gm["re_cas"]
        return gm
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------
    # 2b. Per-point re_eff sanity filter (instrument artifact rejection)
    # ----------------------------------------------------------
    # Single-frame CIP/CAS sensor artifacts can produce non-physical
    # effective radii. Marine Sc cloud-droplet re is bounded by ~30 um
    # (Bennartz 2007 review). Points exceeding this are flagged as
    # instrument artifacts and set to NaN, preserving the rest of the
    # profile data and downstream median statistics.
    ...
    if n_re_cas_bad + n_re_full_bad > 0:
        # Find which profiles host the artifacts (informative log)
