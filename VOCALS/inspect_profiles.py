...
#
# Output: outputs/figures/VOCALS_step03_profile_inspection.png
# =============================================================================
...
df_qc["Datetime"]    = pd.to_datetime(df_qc["UTC"])
df_pts["Datetime"]   = pd.to_datetime(df_pts["UTC"])
...
for _, row in golden.iterrows():
    ...
    alt = df_pts.loc[mask].sort_values("Datetime")["GALT"].values
...
for ax, flt in zip(axes, flights_with_golden):
    ...
    ax.plot(sub_qc["Datetime"], sub_qc["GALT"], color="lightgray", lw=0.6,
            label="_nolegend_", zorder=1)

    # In-cloud points (LWC > 0.05 AND Nc > 10) overlaid
    in_cloud = (sub_qc["LWC_Gerber"] > 0.05) & (sub_qc["Nc_Total"] > 10)
    ax.scatter(sub_qc.loc[in_cloud, "Datetime"], sub_qc.loc[in_cloud, "GALT"],
...
fig.suptitle(
    f"VOCALS 2008 - step03 Golden Profile Inspection  "
...
# Save
# -----------------------------------------------------------------------------
out_path = config.FIG_DIR / "VOCALS_step03_profile_inspection.png"
