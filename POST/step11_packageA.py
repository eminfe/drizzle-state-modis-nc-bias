...
# Data     : {CAMPAIGN}_golden_case.csv  ->  gc (filtered profiles)
# Outputs  : outputs/figures/{CAMPAIGN}_PackageA_main.png
#            outputs/figures/{CAMPAIGN}_PackageA_bonus.png
#            outputs/figures/{CAMPAIGN}_PackageA_bimodal.png
#            outputs/{CAMPAIGN}_PackageA_notes.md
# =============================================================================
...
fig.suptitle(
    f'{config.CAMPAIGN_NAME} {config.CAMPAIGN_YEAR} — Package A: In-Situ Cloud Physics\n'
...
fig2.suptitle(
    f'{config.CAMPAIGN_NAME} {config.CAMPAIGN_YEAR} — Package A: Bonus — Physical Relationships  (colour = drizzle regime)',
...
#
#      ADDITIONAL GUARD: Bimodal only meaningful if BOTH Mod-Low and
#      Mod-High clusters are populated. Single-cluster cases (e.g. MASE
#      where all moderate profiles have Nd > 100) are skipped because
#      KDE artifacts on n=4 produce spurious bimodal-looking curves.
# ============================================================
N_MOD = (gc['drizzle_regime'] == 'Moderate drizzling').sum()
ND_THRESHOLD = 60   # cm^-3, marine clean/polluted boundary

# Pre-check: do we have profiles in BOTH sub-clusters?
if N_MOD >= 4:
    _mod_nd = gc.loc[gc['drizzle_regime'] == 'Moderate drizzling', 'Nd_median']
    _n_low  = (_mod_nd <  ND_THRESHOLD).sum()
    _n_high = (_mod_nd >= ND_THRESHOLD).sum()
else:
    _n_low = _n_high = 0

if N_MOD < 4:
    print(f'\n[INFO] Bimodal analysis skipped — Moderate drizzling: '
          f'{N_MOD} profiles (<4)')
elif _n_low == 0 or _n_high == 0:
    print(f'\n[INFO] Bimodal analysis skipped — moderate-drizzling profiles '
          f'cluster in a single Nd mode')
    print(f'        Mod-Low (<{ND_THRESHOLD}):  n={_n_low}')
    print(f'        Mod-High (>={ND_THRESHOLD}): n={_n_high}')
    print(f'        (KDE on single cluster produces artifact peaks; '
          f'analysis is meaningful only when both clusters are populated)')
else:
    try:
        print("\n" + "=" * 65)
        ...
        # ND_THRESHOLD is defined above with the gate check.
        ...
        fig3.suptitle(
            f'{config.CAMPAIGN_NAME} {config.CAMPAIGN_YEAR} — Package A: Bimodal Analysis — Moderate Drizzling Nd Distribution',
            color=TXT, fontsize=14, fontweight='bold', y=1.02,
        )
        ...
        # 7. NOTES FILE
        # ============================================================
        notes = f"""# {config.CAMPAIGN_NAME} {config.CAMPAIGN_YEAR} — Bimodal Nd in Moderate Drizzling Regime
